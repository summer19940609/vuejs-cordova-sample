        ...
        <van-list v-model="loading" :finished="finished" @load="onLoad" offset="100">
            <div class="topic-item" v-for="( item ) in list" :key="item.tid" @click="go2detail(item.tpcurl)">
                ...
                <div class="img-group" v-if="item.img">
                    <img v-lazy="pic" class="topic-img" :src="`https://img.nga.178.com/attachments/${pic}`" alt v-for="pic in item.img" :key="pic" />
                </div>
...
export default {
    ...
    methods: {
        async handelTopic(fid, page) {
            ...
            const { list, total, err } = await this.getNgaTopic(
                fid,
                String(page),
            );
            if (err) {
            ...
            for (const v of list) {
                const { data, err } = await this.getReadDetail(
                    String(v.tid),
                    '1',
                );
                if (err) {
                ...
                window.cordova.plugin.http.sendRequest(
                    ...
                    (res) => {
                        ...
                        if (Array.isArray(_T)) {
                            ...
                            Object.prototype.toString.call(_T) ===
                            '[object Object]'
                        ) {
                        ...
                        resolve({
                            list: list,
                            total: data.data.__ROWS,
                            err: null,
                        });
                    },
                    (err) => {
                        console.log(
                            `====> [thread.php]的err为: ${JSON.stringify(err)}`,
                        );
        ...
        getReadDetail(tid, page) {
            return new Promise((resolve, reject) => {
                ...
                window.cordova.plugin.http.sendRequest(
                    ...
                    (err) => {
                        console.log(
                            `====> [read.php]的err为: ${JSON.stringify(err)}`,
                        );
                        this.$toast.fail('获取数据失败');
        ...
        async onLoad() {
            ...
            const { list, total, err } = await this.handelTopic(
                this.fid,
                this.page + 1,
            );
            if (err) {
